...
int main(int argc, char *argv[])
{
    if (argc != 4)
    {
        ...
        std::cout << "./program dataset_path output_path threshold" << std::endl;
        return 0;
    }

    uint64_t cnt = 0;
    uint32_t threshold = atoi(argv[3]);

    TUPLES *read_data_tuples = read_data(argv[1], 100000000, &cnt);
    ...
    std::cout << cnt << " " <<  Mp[32].size()<< std::endl;
