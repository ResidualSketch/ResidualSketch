...
struct TUPLES{
    ...
    inline uint64_t srcIP_dstIP_prefix(uint32_t prefix1, uint32_t prefix2) const{
        uint64_t srcMask = (0xFFFFFFFFULL) << (32 - prefix1);
        uint64_t dstMask = (0xFFFFFFFFULL) << (32 - prefix2);
        return ((srcIP() & srcMask) << 32) | (dstIP()  & dstMask);
    }
